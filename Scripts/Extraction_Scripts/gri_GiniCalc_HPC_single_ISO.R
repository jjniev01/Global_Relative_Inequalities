...
## rescaled value raster and calculate the Gini coefficient of such values along with the Interquartile range. Outputs a .RDS file (class of contained object is a data.table) with the calculated Gini coefficient of the extracted values. The R script called below can be modified to use any zonal raster
## but currently has the level 0 national boundaries (as defined by the worldpop geospatial library)
## hardcoded into the script. Allows for the specification of a single ISO code (i.e. country but this could be modified to mean any single zone within a zonal raster).
...
iso_codes <-as.numeric(eval(parse(text=args[5])))
...
cat(sprintf('YEAR: %d \nCORES: %d\nTHRESHOLD: %d\nMAKESUM: %s',
            year, core_number, threshold_val, make_sum))
