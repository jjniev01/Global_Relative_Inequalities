...
#
##  ISO code
#i <- as.numeric(eval(parse(text=args[5])))
...
giniCalcSerial <- function(g){
  ##  Description:  
  ##
  ##  Parameters:
  ##  -  geographic ID (ISOCODE) of the country we are working with whose values
  ##     we want to extract and calculate metrics for.
  ##
  ##  Values:
  ##  -  List of values to return to the main handler for merging with the 
  ##     main data.table of values
...
gini_dt_filled <- clusterGini(gini_dt,zonal_ras,value_ras,value_tot_ras)
endCluster()
# for(i in 1:nrow(gini_dt)){
#   tStart <- Sys.time()
#   ##	Now store our task item (list of three objects) in our task list:
...
##  Now handle the larger countries:
for(i in 1:length(non_shared_isos)){
  tStart <- Sys.time()
  print(paste0("Start time: ", tStart))
  foo_gini_dt <- data.table("ISO_number" = non_shared_isos[i],
                            ##  Gini coefficient
                            "Gini_pop" = numeric(length = 1),
                            ##  25th percentile
                            "P25_pop" = numeric(length = 1),
                            ##  50th percentile
                            "P50_pop" = numeric(length = 1),
                            ##  75th percentile
                            "P75_pop" = numeric(length = 1),
                            ##  Gini coefficient
                            "Gini_urb" = numeric(length = 1),
                            ##  25th percentile
                            "P25_urb" = numeric(length = 1),
                            ##  50th percentile
                            "P50_urb" = numeric(length = 1),
                            ##  75th percentile
                            "P75_urb" = numeric(length = 1),
                            ##  Gini coefficient
                            "Gini_lan" = numeric(length = 1),
                            ##  25th percentile
                            "P25_lan" = numeric(length = 1),
                            ##  50th percentile
                            "P50_lan" = numeric(length = 1),
                            ##  75th percentile
                            "P75_lan" = numeric(length = 1),
                            ##  Gini coefficient
                            "Gini_tot" = numeric(length = 1),
                            ##  25th percentile
                            "P25_tot" = numeric(length = 1),
                            ##  50th percentile
                            "P50_tot" = numeric(length = 1),
                            ##  75th percentile
                            "P75_tot" = numeric(length = 1))
  ##	Now store our task item (list of three objects) in our task list:
  foo_gini_dt[ISO_number == non_shared_isos[i],
          names(gini_dt):= giniCalcSerial(non_shared_isos[i])]
  tEnd <- Sys.time()
  wpProgressMessage(i,
                    max = nrow(foo_gini_dt),
                    label = paste0("Received country ", i,
                                   " Processing Time: ",
                                   wpTimeDiff(tStart,tEnd)))
  saveRDS(gini_dt_filled,
          file = paste0(root,"Output/GRI_Gini_data_",year,"_",threshold_val,"_ISO",non_shared_isos[i],".RDS"))
